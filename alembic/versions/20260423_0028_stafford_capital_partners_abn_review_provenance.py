"""add ABR reviewed identity for Stafford Capital Partners manager seed"""
...
STAFFORD_CAPITAL_PARTNERS_REVIEWED_AT = "2026-04-23"
...
STAFFORD_CAPITAL_PARTNERS_UPDATED_NOTES = (
...
def upgrade() -> None:
    ...
    if len(rows) != 1:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity expected exactly one canonical manager entity"
        )

    row = rows[0]
    if row["entity_type"] != "manager":
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity expected a manager entity, found "
            f"{row['entity_type']!r}"
        )
    if _normalise_abn(row["abn"]) not in {None, _normalise_abn(STAFFORD_CAPITAL_PARTNERS_REVIEWED_ABN)}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found a conflicting ABN "
            f"({row['abn']!r})"
        )
    if row["abn_review_source"] not in {None, STAFFORD_CAPITAL_PARTNERS_REVIEW_SOURCE}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found conflicting abn_review_source "
            f"({row['abn_review_source']!r})"
        )
    if row["abn_reviewed_by"] not in {None, STAFFORD_CAPITAL_PARTNERS_REVIEWED_BY}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found conflicting abn_reviewed_by "
            f"({row['abn_reviewed_by']!r})"
        )
    if row["abn_reviewed_at"] not in {None, date.fromisoformat(STAFFORD_CAPITAL_PARTNERS_REVIEWED_AT)}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found conflicting abn_reviewed_at "
            f"({row['abn_reviewed_at']!r})"
        )
    if row["registered_name_on_abr"] not in {None, STAFFORD_CAPITAL_PARTNERS_REGISTERED_NAME_ON_ABR}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found conflicting registered_name_on_abr "
            f"({row['registered_name_on_abr']!r})"
        )
    if row["country_code"] not in {None, "", "AU"}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found conflicting country_code "
            f"({row['country_code']!r})"
        )
    if row["is_australian_entity"] is False:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found is_australian_entity=False"
        )
    if row["confidence_tier"] not in {None, "seeded"}:
        raise RuntimeError(
            "Stafford Capital Partners reviewed identity found conflicting confidence_tier "
    ...
    bind.execute(
        ...
        ),
        {
            ...
            "review_source": STAFFORD_CAPITAL_PARTNERS_REVIEW_SOURCE,
            "reviewed_by": STAFFORD_CAPITAL_PARTNERS_REVIEWED_BY,
            "reviewed_at": STAFFORD_CAPITAL_PARTNERS_REVIEWED_AT,
            "registered_name_on_abr": STAFFORD_CAPITAL_PARTNERS_REGISTERED_NAME_ON_ABR,
            "is_australian_entity": True,
            "country_code": "AU",
            "confidence_tier": "seeded",
            "legacy_notes": STAFFORD_CAPITAL_PARTNERS_LEGACY_NOTES,
            "updated_notes": STAFFORD_CAPITAL_PARTNERS_UPDATED_NOTES,
            "entity_id": row["id"],
        },
    )
    ...
    bind.execute(
        ...
        {
            "entity_id": row["id"],
            "alias_insert": STAFFORD_CAPITAL_PARTNERS_CURRENT_LEGAL_ALIAS,
            "alias_lookup": STAFFORD_CAPITAL_PARTNERS_CURRENT_LEGAL_ALIAS,
            "alias_normalized": "stafford capital partners pty ltd",
...
def downgrade() -> None:
    ...
    bind.execute(
        ...
        {
            "entity_id": entity_id,
            "alias": STAFFORD_CAPITAL_PARTNERS_CURRENT_LEGAL_ALIAS,
        },
    )
