"""add ABR reviewed identity for Alphinity Investment Management manager seed"""
...
ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_AT = "2026-04-22"
...
ALPHINITY_INVESTMENT_MANAGEMENT_UPDATED_NOTES = (
...
def upgrade() -> None:
    ...
    if len(rows) != 1:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity expected exactly one canonical manager entity"
        )

    row = rows[0]
    if row["entity_type"] != "manager":
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity expected a manager entity, found "
            f"{row['entity_type']!r}"
        )
    if _normalise_abn(row["abn"]) not in {None, _normalise_abn(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_ABN)}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found a conflicting ABN "
            f"({row['abn']!r})"
        )
    if row["abn_review_source"] not in {None, ALPHINITY_INVESTMENT_MANAGEMENT_REVIEW_SOURCE}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found conflicting abn_review_source "
            f"({row['abn_review_source']!r})"
        )
    if row["abn_reviewed_by"] not in {None, ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_BY}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found conflicting abn_reviewed_by "
            f"({row['abn_reviewed_by']!r})"
        )
    if row["abn_reviewed_at"] not in {None, date.fromisoformat(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_AT)}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found conflicting abn_reviewed_at "
            f"({row['abn_reviewed_at']!r})"
        )
    if row["registered_name_on_abr"] not in {None, ALPHINITY_INVESTMENT_MANAGEMENT_REGISTERED_NAME_ON_ABR}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found conflicting registered_name_on_abr "
            f"({row['registered_name_on_abr']!r})"
        )
    if row["country_code"] not in {None, "", "AU"}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found conflicting country_code "
            f"({row['country_code']!r})"
        )
    if row["is_australian_entity"] is False:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found is_australian_entity=False"
        )
    if row["confidence_tier"] not in {None, "seeded"}:
        raise RuntimeError(
            "Alphinity Investment Management reviewed identity found conflicting confidence_tier "
    ...
    bind.execute(
        ...
        ),
        {
            ...
            "review_source": ALPHINITY_INVESTMENT_MANAGEMENT_REVIEW_SOURCE,
            "reviewed_by": ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_BY,
            "reviewed_at": ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_AT,
            ...
            "legacy_notes": ALPHINITY_INVESTMENT_MANAGEMENT_LEGACY_NOTES,
            "updated_notes": ALPHINITY_INVESTMENT_MANAGEMENT_UPDATED_NOTES,
            "entity_id": row["id"],
        },
    )
    ...
    bind.execute(
        ...
        {
            "entity_id": row["id"],
            "alias_insert": ALPHINITY_INVESTMENT_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
            "alias_lookup": ALPHINITY_INVESTMENT_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
            "alias_normalized": "challenger alpha aust shares afsl pty limited",
            "source_system": "alphinity-investment-management-stage5-v1",
            "is_preferred": False,
...
def downgrade() -> None:
    ...
    bind.execute(
        ...
        {
            "entity_id": entity_id,
            "alias": ALPHINITY_INVESTMENT_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
        },
    )
