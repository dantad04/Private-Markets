"""backfill ASIC company-register cross-reference for Marron Group Holdings Pty Ltd"""
...
from app.entity_resolution.private_entity_asic_company_register_cross_reference import (
    ...
    ensure_marron_group_holdings_asic_company_register_cross_reference_subset,
)
...
def upgrade() -> None:
    ...
    try:
        ensure_marron_group_holdings_asic_company_register_cross_reference_subset(session)
        session.commit()
    finally:
        session.close()
...
def downgrade() -> None:
    ...
    try:
        session.execute(
            sa.update(Entity)
            .where(Entity.canonical_name == MARRON_GROUP_HOLDINGS_CANONICAL_NAME)
            .values(
                abn=None,
                country_code=None,
