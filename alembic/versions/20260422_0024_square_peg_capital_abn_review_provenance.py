"""add ABR reviewed identity for Square Peg Capital manager seed"""
...
SQUARE_PEG_CAPITAL_REVIEWED_AT = "2026-04-22"
...
SQUARE_PEG_CAPITAL_UPDATED_NOTES = (
...
def upgrade() -> None:
    ...
    if len(rows) != 1:
        raise RuntimeError("Square Peg Capital reviewed identity expected exactly one canonical manager entity")

    row = rows[0]
    if row["entity_type"] != "manager":
        raise RuntimeError(
            "Square Peg Capital reviewed identity expected a manager entity, found "
            f"{row['entity_type']!r}"
        )
    if _normalise_abn(row["abn"]) not in {None, _normalise_abn(SQUARE_PEG_CAPITAL_REVIEWED_ABN)}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found a conflicting ABN "
            f"({row['abn']!r})"
        )
    if row["abn_review_source"] not in {None, SQUARE_PEG_CAPITAL_REVIEW_SOURCE}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found conflicting abn_review_source "
            f"({row['abn_review_source']!r})"
        )
    if row["abn_reviewed_by"] not in {None, SQUARE_PEG_CAPITAL_REVIEWED_BY}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found conflicting abn_reviewed_by "
            f"({row['abn_reviewed_by']!r})"
        )
    if row["abn_reviewed_at"] not in {None, date.fromisoformat(SQUARE_PEG_CAPITAL_REVIEWED_AT)}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found conflicting abn_reviewed_at "
            f"({row['abn_reviewed_at']!r})"
        )
    if row["registered_name_on_abr"] not in {None, SQUARE_PEG_CAPITAL_REGISTERED_NAME_ON_ABR}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found conflicting registered_name_on_abr "
            f"({row['registered_name_on_abr']!r})"
        )
    if row["country_code"] not in {None, "", "AU"}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found conflicting country_code "
            f"({row['country_code']!r})"
        )
    if row["is_australian_entity"] is False:
        raise RuntimeError("Square Peg Capital reviewed identity found is_australian_entity=False")
    if row["confidence_tier"] not in {None, "seeded"}:
        raise RuntimeError(
            "Square Peg Capital reviewed identity found conflicting confidence_tier "
    ...
    bind.execute(
        ...
        ),
        {
            ...
            "review_source": SQUARE_PEG_CAPITAL_REVIEW_SOURCE,
            "reviewed_by": SQUARE_PEG_CAPITAL_REVIEWED_BY,
            "reviewed_at": SQUARE_PEG_CAPITAL_REVIEWED_AT,
            "registered_name_on_abr": SQUARE_PEG_CAPITAL_REGISTERED_NAME_ON_ABR,
            "is_australian_entity": True,
            "country_code": "AU",
            "confidence_tier": "seeded",
            "legacy_notes": SQUARE_PEG_CAPITAL_LEGACY_NOTES,
            "updated_notes": SQUARE_PEG_CAPITAL_UPDATED_NOTES,
            "entity_id": row["id"],
        },
    )
    ...
    bind.execute(
        ...
        {
            "entity_id": row["id"],
            "alias_insert": SQUARE_PEG_CAPITAL_CURRENT_LEGAL_ALIAS,
            "alias_lookup": SQUARE_PEG_CAPITAL_CURRENT_LEGAL_ALIAS,
            "alias_normalized": "square peg capital pty ltd",
