"""add ABR reviewed identity for Brandon Capital Partners manager seed"""
...
BRANDON_CAPITAL_PARTNERS_REVIEWED_AT = "2026-04-22"
...
BRANDON_CAPITAL_PARTNERS_UPDATED_NOTES = (
...
def upgrade() -> None:
    ...
    if len(rows) != 1:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity expected exactly one canonical manager entity"
        )

    row = rows[0]
    if row["entity_type"] != "manager":
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity expected a manager entity, found "
            f"{row['entity_type']!r}"
        )
    if _normalise_abn(row["abn"]) not in {None, _normalise_abn(BRANDON_CAPITAL_PARTNERS_REVIEWED_ABN)}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found a conflicting ABN "
            f"({row['abn']!r})"
        )
    if row["abn_review_source"] not in {None, BRANDON_CAPITAL_PARTNERS_REVIEW_SOURCE}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found conflicting abn_review_source "
            f"({row['abn_review_source']!r})"
        )
    if row["abn_reviewed_by"] not in {None, BRANDON_CAPITAL_PARTNERS_REVIEWED_BY}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found conflicting abn_reviewed_by "
            f"({row['abn_reviewed_by']!r})"
        )
    if row["abn_reviewed_at"] not in {None, date.fromisoformat(BRANDON_CAPITAL_PARTNERS_REVIEWED_AT)}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found conflicting abn_reviewed_at "
            f"({row['abn_reviewed_at']!r})"
        )
    if row["registered_name_on_abr"] not in {None, BRANDON_CAPITAL_PARTNERS_REGISTERED_NAME_ON_ABR}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found conflicting registered_name_on_abr "
            f"({row['registered_name_on_abr']!r})"
        )
    if row["country_code"] not in {None, "", "AU"}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found conflicting country_code "
            f"({row['country_code']!r})"
        )
    if row["is_australian_entity"] is False:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found is_australian_entity=False"
        )
    if row["confidence_tier"] not in {None, "seeded"}:
        raise RuntimeError(
            "Brandon Capital Partners reviewed identity found conflicting confidence_tier "
    ...
    bind.execute(
        ...
        ),
        {
            ...
            "review_source": BRANDON_CAPITAL_PARTNERS_REVIEW_SOURCE,
            "reviewed_by": BRANDON_CAPITAL_PARTNERS_REVIEWED_BY,
            "reviewed_at": BRANDON_CAPITAL_PARTNERS_REVIEWED_AT,
            "registered_name_on_abr": BRANDON_CAPITAL_PARTNERS_REGISTERED_NAME_ON_ABR,
            "is_australian_entity": True,
            "country_code": "AU",
            "confidence_tier": "seeded",
            "legacy_notes": BRANDON_CAPITAL_PARTNERS_LEGACY_NOTES,
            "updated_notes": BRANDON_CAPITAL_PARTNERS_UPDATED_NOTES,
            "entity_id": row["id"],
        },
    )
    ...
    bind.execute(
        ...
        {
            "entity_id": row["id"],
            "alias_insert": BRANDON_CAPITAL_PARTNERS_CURRENT_LEGAL_ALIAS,
            "alias_lookup": BRANDON_CAPITAL_PARTNERS_CURRENT_LEGAL_ALIAS,
            "alias_normalized": "brandon capital partners pty ltd",
            "source_system": "brandon-capital-partners-stage5-v1",
            "is_preferred": False,
...
def downgrade() -> None:
    ...
    bind.execute(
        ...
        {
            "entity_id": entity_id,
            "alias": BRANDON_CAPITAL_PARTNERS_CURRENT_LEGAL_ALIAS,
        },
    )
