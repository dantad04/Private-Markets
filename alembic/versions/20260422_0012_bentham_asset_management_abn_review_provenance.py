"""add ABR reviewed identity for Bentham Asset Management manager seed"""
...
BENTHAM_ASSET_MANAGEMENT_REVIEWED_AT = "2026-04-22"
...
BENTHAM_ASSET_MANAGEMENT_UPDATED_NOTES = (
...
def upgrade() -> None:
    ...
    if len(rows) != 1:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity expected exactly one canonical manager entity"
        )

    row = rows[0]
    if row["entity_type"] != "manager":
        raise RuntimeError(
            "Bentham Asset Management reviewed identity expected a manager entity, found "
            f"{row['entity_type']!r}"
        )
    if _normalise_abn(row["abn"]) not in {None, _normalise_abn(BENTHAM_ASSET_MANAGEMENT_REVIEWED_ABN)}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found a conflicting ABN "
            f"({row['abn']!r})"
        )
    if row["abn_review_source"] not in {None, BENTHAM_ASSET_MANAGEMENT_REVIEW_SOURCE}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found conflicting abn_review_source "
            f"({row['abn_review_source']!r})"
        )
    if row["abn_reviewed_by"] not in {None, BENTHAM_ASSET_MANAGEMENT_REVIEWED_BY}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found conflicting abn_reviewed_by "
            f"({row['abn_reviewed_by']!r})"
        )
    if row["abn_reviewed_at"] not in {None, date.fromisoformat(BENTHAM_ASSET_MANAGEMENT_REVIEWED_AT)}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found conflicting abn_reviewed_at "
            f"({row['abn_reviewed_at']!r})"
        )
    if row["registered_name_on_abr"] not in {None, BENTHAM_ASSET_MANAGEMENT_REGISTERED_NAME_ON_ABR}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found conflicting registered_name_on_abr "
            f"({row['registered_name_on_abr']!r})"
        )
    if row["country_code"] not in {None, "", "AU"}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found conflicting country_code "
            f"({row['country_code']!r})"
        )
    if row["is_australian_entity"] is False:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found is_australian_entity=False"
        )
    if row["confidence_tier"] not in {None, "seeded"}:
        raise RuntimeError(
            "Bentham Asset Management reviewed identity found conflicting confidence_tier "
    ...
    bind.execute(
        ...
        ),
        {
            ...
            "review_source": BENTHAM_ASSET_MANAGEMENT_REVIEW_SOURCE,
            "reviewed_by": BENTHAM_ASSET_MANAGEMENT_REVIEWED_BY,
            "reviewed_at": BENTHAM_ASSET_MANAGEMENT_REVIEWED_AT,
            "registered_name_on_abr": BENTHAM_ASSET_MANAGEMENT_REGISTERED_NAME_ON_ABR,
            "is_australian_entity": True,
            "country_code": "AU",
            "confidence_tier": "seeded",
            "legacy_notes": BENTHAM_ASSET_MANAGEMENT_LEGACY_NOTES,
            "updated_notes": BENTHAM_ASSET_MANAGEMENT_UPDATED_NOTES,
            "entity_id": row["id"],
        },
    )
    ...
    bind.execute(
        ...
        {
            "entity_id": row["id"],
            "alias_insert": BENTHAM_ASSET_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
            "alias_lookup": BENTHAM_ASSET_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
            "alias_normalized": "challenger alpha re afsl pty limited",
            "source_system": "bentham-asset-management-stage5-v1",
            "is_preferred": False,
...
def downgrade() -> None:
    ...
    bind.execute(
        ...
        {
            "entity_id": entity_id,
            "alias": BENTHAM_ASSET_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
        },
    )
