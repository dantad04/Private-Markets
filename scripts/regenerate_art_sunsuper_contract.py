...
FIXTURE_PATH = Path("tests/fixtures/art_sunsuper_synthetic_stable_minimal.csv")
OUTPUT_PATH = Path("tests/adapters/contracts/art_sunsuper/canonical_output.json")
WARNING_TEXT = "THIS REWRITES THE STAGE 2 ART-SUNSUPER CONTRACT. COMMIT THE CHANGE WITH AN EXPLICIT MESSAGE EXPLAINING WHY."


def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Regenerate the frozen ART-Sunsuper adapter contract artifact.")
...
def make_contract_metadata() -> SourceFileMetadata:
    return SourceFileMetadata(
        source_file_id="contract-fixture-art-sunsuper",
        fund_id="art",
        suspected_adapter_key="ArtSunsuperPhdAdapter",
        reporting_period_id=1,
        source_url=str(FIXTURE_PATH),
        checksum="contract-fixture-art-sunsuper-sha256-placeholder",
...
def main() -> int:
    ...
    result = ArtSunsuperPhdAdapter().parse(make_contract_metadata(), FIXTURE_PATH.read_bytes())
