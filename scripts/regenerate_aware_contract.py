...
FIXTURE_PATH = Path("tests/fixtures/aware_synthetic_table1_minimal.csv")
OUTPUT_PATH = Path("tests/adapters/contracts/aware/canonical_output.json")
WARNING_TEXT = "THIS REWRITES THE STAGE 2 AWARE CONTRACT. COMMIT THE CHANGE WITH AN EXPLICIT MESSAGE EXPLAINING WHY."
...
def make_contract_metadata() -> SourceFileMetadata:
    return SourceFileMetadata(
        source_file_id="contract-fixture-aware",
        fund_id="aware",
        suspected_adapter_key="AwarePhdAdapter",
        reporting_period_id=None,
        source_url=str(FIXTURE_PATH),
        checksum="contract-fixture-aware-sha256-placeholder",
...
def main() -> int:
    ...
    result = AwarePhdAdapter().parse(make_contract_metadata(), FIXTURE_PATH.read_bytes())
