...
FIXTURE_PATH = Path("tests/fixtures/art_qsuper_synthetic_balanced_minimal.csv")
OUTPUT_PATH = Path("tests/adapters/contracts/art_qsuper/canonical_output.json")
WARNING_TEXT = "THIS REWRITES THE STAGE 2 ART-QSUPER CONTRACT. COMMIT THE CHANGE WITH AN EXPLICIT MESSAGE EXPLAINING WHY."


def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Regenerate the frozen ART-QSuper adapter contract artifact.")
    parser.add_argument(
        "--confirm",
        action="store_true",
...
def make_contract_metadata() -> SourceFileMetadata:
    return SourceFileMetadata(
        source_file_id="contract-fixture-art-qsuper",
        fund_id="art",
        suspected_adapter_key="ArtQsuperPhdAdapter",
        reporting_period_id=None,
        source_url=str(FIXTURE_PATH),
        checksum="contract-fixture-art-qsuper-sha256-placeholder",
...
def main() -> int:
    ...
    result = ArtQsuperPhdAdapter().parse(make_contract_metadata(), FIXTURE_PATH.read_bytes())
