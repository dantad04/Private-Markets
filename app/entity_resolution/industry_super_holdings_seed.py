...
def ensure_industry_super_holdings_seed(session) -> Entity:
    ...
    if entity is None:
        entity = Entity(
            ...
            notes=(
                "Stage 3 Industry Super Holdings canonical seed. ABN intentionally left null in this run "
                "because it was not verified from approved repo-backed source material."
            ),
        )
        session.add(entity)
        session.flush()
    ...
    for alias, is_preferred in INDUSTRY_SUPER_HOLDINGS_OBSERVED_ALIASES:
        ...
        session.add(
            EntityAlias(
                ...
                source_system="industry-super-holdings-stage3-v1",
                source_file_id=None,
