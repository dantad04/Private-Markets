...
ROC_CAPITAL_OBSERVED_ALIASES: tuple[tuple[str, bool], ...] = (
    ...
    ("ROC Capital Pty Ltd", False),
)


def ensure_roc_capital_seed(session) -> Entity:
    entity = session.scalar(
        select(Entity).where(
            Entity.canonical_name == ROC_CAPITAL_CANONICAL_NAME,
        )
    )
    if entity is None:
        entity = Entity(
    ...
    _ensure_roc_capital_seed_shape(entity)
    ...
    for alias, is_preferred in ROC_CAPITAL_OBSERVED_ALIASES:
        ...
        session.add(
            EntityAlias(
                ...
                source_system=ROC_CAPITAL_SEED_SOURCE,
                source_file_id=None,
...
def _ensure_roc_capital_seed_shape(entity: Entity) -> None:
