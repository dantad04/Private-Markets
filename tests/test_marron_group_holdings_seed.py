...
from app.entity_resolution.private_entity_asic_company_register_cross_reference import (
    ...
    MARRON_GROUP_HOLDINGS_ASIC_CROSS_REFERENCE,
...
class TestMarronGroupHoldingsAsicCrossReference(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.tempdir = tempfile.TemporaryDirectory()
        cls.database_url = f"sqlite:///{Path(cls.tempdir.name) / 'marron_group_holdings_asic_cross_reference.db'}"
        cls.engine = get_engine(cls.database_url)
        Base.metadata.create_all(cls.engine)
        cls.SessionLocal = sessionmaker(bind=cls.engine, autoflush=False, autocommit=False, future=True)
        ...
        with cls.SessionLocal() as session:
            ...
            cls.pre_seed_signatures = TestMarronGroupHoldingsSeed._load_marron_signatures(session)
            cls.pre_seed_null_entity_count = session.scalar(
                select(func.count(Holding.id)).where(
                    Holding.raw_name == MARRON_GROUP_HOLDINGS_CANONICAL_NAME,
                    Holding.entity_id.is_(None),
    ...
    def test_canonical_company_seed_persists_asic_cross_reference_without_abr_review_layer(self) -> None:
        with self.SessionLocal() as session:
            ...
            self.assertEqual(MARRON_GROUP_HOLDINGS_CANONICAL_NAME, entity.canonical_name)
            self.assertEqual("company", entity.entity_type)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ABN, entity.abn)
            self.assertIsNone(entity.abn_review_source)
            self.assertIsNone(entity.registered_name_on_abr)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ACN, entity.acn)
            ...
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_REGISTRATION_DATE, entity.asic_registration_date)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_NEXT_REVIEW_DATE, entity.asic_next_review_date)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_CROSS_REFERENCE.record_url, entity.asic_record_url)
            self.assertEqual(ASIC_REVIEW_SOURCE, entity.asic_review_source)
            self.assertEqual(ASIC_REVIEWED_BY, entity.asic_reviewed_by)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_REVIEWED_AT, entity.asic_reviewed_at)
    ...
    def test_seed_is_idempotent_and_reapplies_only_marron_asic_fields(self) -> None:
        with self.SessionLocal() as session:
            first = ensure_marron_group_holdings_seed(session)
            second = ensure_marron_group_holdings_seed(session)
            ensure_marron_group_holdings_asic_company_register_cross_reference(session)
    ...
    def test_exact_name_resolution_keeps_warrants_untouched_and_company_detail_stays_honest_with_asic_only_slice(
        self,
    ) -> None:
        with self.SessionLocal() as session:
            company_rows = session.scalars(
            ...
            self.assertEqual(0, self.first_resolution_summary.exact_name_candidates_rejected_due_to_scope_conflict)
            self.assertTrue(all(holding.entity_id == self.entity_id for holding in company_rows))
            self.assertEqual(self.pre_seed_signatures, TestMarronGroupHoldingsSeed._load_marron_signatures(session))
            ...
            self.assertTrue(detail.history_is_limited)
            self.assertEqual("Linked", detail.entity_confidence_label)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ABN, detail.abn)
            self.assertIsNone(detail.abn_review_source)
            self.assertIsNone(detail.registered_name_on_abr)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ACN, detail.acn)
            ...
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_REGISTRATION_DATE, detail.asic_registration_date)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_NEXT_REVIEW_DATE, detail.asic_next_review_date)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_CROSS_REFERENCE.record_url, detail.asic_record_url)
            self.assertEqual(ASIC_REVIEW_SOURCE, detail.asic_review_source)
            self.assertEqual(ASIC_REVIEWED_BY, detail.asic_reviewed_by)
            self.assertEqual(MARRON_GROUP_HOLDINGS_ASIC_REVIEWED_AT, detail.asic_reviewed_at)
