...
class TestCompanyDetailApi(unittest.TestCase):
    ...
    def test_company_detail_endpoint_returns_canonical_identity_aliases_and_honest_history_shape(self) -> None:
        ...
        self.assertEqual([], payload["manager_role_rows"])
        self.assertEqual([], payload["issuer_role_rows"])
        self.assertEqual(5, len(payload["direct_holding_rows"]))
        self.assertEqual(
            [
                {
    ...
    def test_company_detail_endpoint_does_not_fabricate_relationships_and_admin_ui_renders_same_data(self) -> None:
        ...
        self.assertIn("14.32%", ui_response.text)
        self.assertIn("13.17%", ui_response.text)
        self.assertIn("Confidence: Reviewed", ui_response.text)
        self.assertIn("Disclosed as held entity", ui_response.text)
        self.assertNotIn("Disclosed as manager", ui_response.text)
        self.assertNotIn("Disclosed as issuer", ui_response.text)
