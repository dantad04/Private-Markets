...
from app.entity_resolution.private_entity_asic_company_register_cross_reference import (
    BENTHAM_ASSET_MANAGEMENT_ACN,
    BENTHAM_ASSET_MANAGEMENT_ASIC_CROSS_REFERENCE,
    BENTHAM_ASSET_MANAGEMENT_ASIC_NEXT_REVIEW_DATE,
    BENTHAM_ASSET_MANAGEMENT_ASIC_REGISTRATION_DATE,
    ASIC_COMPANY_STATUS_REGISTERED,
    ASIC_COMPANY_TYPE_PROPRIETARY_LIMITED_BY_SHARES,
    ASIC_REVIEWED_AT,
    ASIC_REVIEWED_BY,
    ASIC_REVIEW_SOURCE,
    ensure_bentham_asset_management_asic_company_register_cross_reference,
)
...
class TestBenthamAssetManagementSeed(unittest.TestCase):
    ...
    def test_canonical_manager_seed_persists_reviewed_identity_in_place_and_is_idempotent(self) -> None:
        with self.SessionLocal() as session:
            ...
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ACN, entity.acn)
            ...
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ASIC_REGISTRATION_DATE, entity.asic_registration_date)
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ASIC_NEXT_REVIEW_DATE, entity.asic_next_review_date)
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ASIC_CROSS_REFERENCE.record_url, entity.asic_record_url)
    ...
    def test_manager_detail_keeps_private_equity_name_only_rows_as_unknown_supplemental_observations(self) -> None:
        with self.SessionLocal() as session:
            ...
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ACN, detail.acn)
            ...
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ASIC_REGISTRATION_DATE, detail.asic_registration_date)
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ASIC_NEXT_REVIEW_DATE, detail.asic_next_review_date)
            self.assertEqual(BENTHAM_ASSET_MANAGEMENT_ASIC_CROSS_REFERENCE.record_url, detail.asic_record_url)
