...
from app.entity_resolution.alphinity_investment_management_seed import (
    ALPHINITY_INVESTMENT_MANAGEMENT_ALIASES,
    ALPHINITY_INVESTMENT_MANAGEMENT_CANONICAL_NAME,
    ALPHINITY_INVESTMENT_MANAGEMENT_HISTORICAL_NAME_ON_ABR,
    ...
    ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_AT,
    ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_BY,
...
from app.entity_resolution.private_entity_asic_company_register_cross_reference import (
    ALPHINITY_INVESTMENT_MANAGEMENT_ACN,
    ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_CROSS_REFERENCE,
    ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_NEXT_REVIEW_DATE,
    ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_REGISTRATION_DATE,
    ASIC_COMPANY_STATUS_REGISTERED,
    ASIC_COMPANY_TYPE_PROPRIETARY_LIMITED_BY_SHARES,
    ASIC_REVIEWED_AT,
    ASIC_REVIEWED_BY,
    ASIC_REVIEW_SOURCE,
    ensure_alphinity_investment_management_asic_company_register_cross_reference,
)
...
class TestAlphinityInvestmentManagementSeed(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.tempdir = tempfile.TemporaryDirectory()
        cls.database_url = f"sqlite:///{Path(cls.tempdir.name) / 'alphinity_investment_management_seed.db'}"
        ...
        with cls.SessionLocal() as session:
            ...
            entity = ensure_alphinity_investment_management_seed(session)
    ...
    def test_canonical_manager_seed_persists_reviewed_identity_in_place_and_is_idempotent(self) -> None:
        with self.SessionLocal() as session:
            first = ensure_alphinity_investment_management_seed(session)
            second = ensure_alphinity_investment_management_seed(session)
            ensure_alphinity_investment_management_asic_company_register_cross_reference(session)
            ensure_alphinity_investment_management_asic_company_register_cross_reference(session)
            session.commit()
            ...
            self.assertEqual(
                len(ALPHINITY_INVESTMENT_MANAGEMENT_ALIASES),
                session.scalar(
            ...
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_CANONICAL_NAME, entity.canonical_name)
            self.assertEqual("manager", entity.entity_type)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_ABN, entity.abn)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEW_SOURCE, entity.abn_review_source)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_BY, entity.abn_reviewed_by)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_AT, entity.abn_reviewed_at)
            self.assertEqual(
                ALPHINITY_INVESTMENT_MANAGEMENT_REGISTERED_NAME_ON_ABR,
                entity.registered_name_on_abr,
            )
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ACN, entity.acn)
            ...
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_REGISTRATION_DATE, entity.asic_registration_date)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_NEXT_REVIEW_DATE, entity.asic_next_review_date)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_CROSS_REFERENCE.record_url, entity.asic_record_url)
            ...
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_NOTES, entity.notes)
            ...
            self.assertEqual(
                [alias for alias, _is_preferred in ALPHINITY_INVESTMENT_MANAGEMENT_ALIASES],
                aliases,
            )

    def test_deterministic_resolution_links_current_alphinity_manager_rows_and_reruns_idempotently(self) -> None:
        with self.SessionLocal() as session:
            rows = session.scalars(
                select(Holding)
                .where(Holding.raw_name == ALPHINITY_INVESTMENT_MANAGEMENT_CANONICAL_NAME)
    ...
    def test_manager_detail_renders_current_manager_rows_with_reviewed_identity_fields(self) -> None:
        with self.SessionLocal() as session:
            ...
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_CANONICAL_NAME, detail.canonical_name)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_ABN, detail.abn)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEW_SOURCE, detail.abn_review_source)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_BY, detail.abn_reviewed_by)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_REVIEWED_AT, detail.abn_reviewed_at)
            self.assertEqual(
                ALPHINITY_INVESTMENT_MANAGEMENT_REGISTERED_NAME_ON_ABR,
                detail.registered_name_on_abr,
            )
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ACN, detail.acn)
            ...
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_REGISTRATION_DATE, detail.asic_registration_date)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_NEXT_REVIEW_DATE, detail.asic_next_review_date)
            self.assertEqual(ALPHINITY_INVESTMENT_MANAGEMENT_ASIC_CROSS_REFERENCE.record_url, detail.asic_record_url)
            ...
            self.assertEqual([ALPHINITY_INVESTMENT_MANAGEMENT_CANONICAL_NAME], detail.matched_raw_names)
            ...
            self.assertEqual(
                {
                    ("Stable", 1007, "value_only", "manager_rollup"),
                    ("Conservative Balanced", 1007, "value_only", "manager_rollup"),
                },
                primary_rows,
            )
