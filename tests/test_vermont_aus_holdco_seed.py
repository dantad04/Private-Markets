...
from app.entity_resolution.private_entity_asic_company_register_cross_reference import (
    ...
    ASIC_REVIEWED_BY,
    ASIC_REVIEW_SOURCE,
    VERMONT_AUS_HOLDCO_ABN,
    VERMONT_AUS_HOLDCO_ACN,
    VERMONT_AUS_HOLDCO_ASIC_CROSS_REFERENCE,
    VERMONT_AUS_HOLDCO_ASIC_NEXT_REVIEW_DATE,
    VERMONT_AUS_HOLDCO_ASIC_REGISTRATION_DATE,
    VERMONT_AUS_HOLDCO_ASIC_REVIEWED_AT,
    ensure_vermont_aus_holdco_asic_company_register_cross_reference,
)
from app.entity_resolution.vermont_aus_holdco_seed import (
    VERMONT_AUS_HOLDCO_CANONICAL_NAME,
    VERMONT_AUS_HOLDCO_NOTES,
    VERMONT_AUS_HOLDCO_OBSERVED_ALIASES,
...
class VermontAusHoldcoObservationSignature:
    raw_name: str
    source_file_id: int
...
class TestVermontAusHoldcoSeed(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.tempdir = tempfile.TemporaryDirectory()
        cls.database_url = f"sqlite:///{Path(cls.tempdir.name) / 'vermont_aus_holdco_seed.db'}"
        ...
        with cls.SessionLocal() as session:
            ...
            cls.pre_seed_signatures = cls._load_vermont_signatures(session)
            cls.pre_seed_null_entity_count = session.scalar(
                select(func.count(Holding.id)).where(
                    Holding.raw_name == VERMONT_AUS_HOLDCO_CANONICAL_NAME,
                    Holding.entity_id.is_(None),
                )
            )

            entity = ensure_vermont_aus_holdco_seed(session)
    ...
    @classmethod
    def _load_vermont_signatures(cls, session) -> list[VermontAusHoldcoObservationSignature]:
        rows = session.scalars(
            select(Holding)
            .where(Holding.raw_name == VERMONT_AUS_HOLDCO_CANONICAL_NAME)
            .order_by(Holding.source_file_id.asc(), Holding.source_row_number.asc())
        ).all()
        return [
            VermontAusHoldcoObservationSignature(
    ...
    def test_canonical_company_seed_exists_with_exact_observed_alias_only(self) -> None:
        with self.SessionLocal() as session:
            ...
            self.assertEqual(VERMONT_AUS_HOLDCO_CANONICAL_NAME, entity.canonical_name)
            ...
            self.assertEqual(VERMONT_AUS_HOLDCO_NOTES, entity.notes)
            ...
            self.assertEqual(
                [alias for alias, _is_preferred in VERMONT_AUS_HOLDCO_OBSERVED_ALIASES],
                aliases,
            )

            stable_text = AUSTRALIANSUPER_STABLE_FIXTURE_PATH.read_text(errors="ignore")
            conservative_text = AUSTRALIANSUPER_CONSERVATIVE_FIXTURE_PATH.read_text(errors="ignore")
            for alias in aliases:
                self.assertIn(alias, stable_text)
                self.assertIn(alias, conservative_text)

    def test_seed_is_idempotent_and_does_not_populate_reviewed_identity_or_asic_fields(self) -> None:
        with self.SessionLocal() as session:
            first = ensure_vermont_aus_holdco_seed(session)
            second = ensure_vermont_aus_holdco_seed(session)
            session.commit()
            ...
            self.assertEqual(
                len(VERMONT_AUS_HOLDCO_OBSERVED_ALIASES),
                session.scalar(
    ...
    def test_exact_name_resolution_links_vermont_and_company_detail_stays_dependency_only(self) -> None:
        with self.SessionLocal() as session:
            rows = session.scalars(
                select(Holding)
                .where(Holding.raw_name == VERMONT_AUS_HOLDCO_CANONICAL_NAME)
            ...
            self.assertEqual(self.pre_seed_signatures, self._load_vermont_signatures(session))
            ...
            self.assertEqual(VERMONT_AUS_HOLDCO_CANONICAL_NAME, detail.canonical_name)
            self.assertEqual("company", detail.entity_type)
            self.assertEqual(["Vermont Aus Holdco Pty Ltd"], detail.aliases)
            self.assertEqual(["Vermont Aus Holdco Pty Ltd"], detail.matched_raw_names)
            ...
            self.assertEqual("Vermont Aus Holdco Pty Ltd", stable_observation.raw_name)
            self.assertEqual("ownership_only", stable_observation.disclosure_completeness)
            self.assertEqual(Decimal("0.0011"), stable_observation.ownership_pct)
            ...
            self.assertEqual("Vermont Aus Holdco Pty Ltd", conservative_observation.raw_name)
            self.assertEqual("ownership_only", conservative_observation.disclosure_completeness)
            self.assertEqual(Decimal("0.0028"), conservative_observation.ownership_pct)
...
class TestVermontAusHoldcoAsicCrossReference(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.tempdir = tempfile.TemporaryDirectory()
        cls.database_url = f"sqlite:///{Path(cls.tempdir.name) / 'vermont_aus_holdco_asic_cross_reference.db'}"
        cls.engine = get_engine(cls.database_url)
        Base.metadata.create_all(cls.engine)
        cls.SessionLocal = sessionmaker(bind=cls.engine, autoflush=False, autocommit=False, future=True)
        ...
        with cls.SessionLocal() as session:
            ...
            cls.pre_seed_signatures = TestVermontAusHoldcoSeed._load_vermont_signatures(session)
            cls.pre_seed_null_entity_count = session.scalar(
                select(func.count(Holding.id)).where(
                    Holding.raw_name == VERMONT_AUS_HOLDCO_CANONICAL_NAME,
                    Holding.entity_id.is_(None),
            ...
            entity = ensure_vermont_aus_holdco_seed(session)
            ensure_vermont_aus_holdco_asic_company_register_cross_reference(session)
            cls.entity_id = entity.id
            cls.period_id = period.id
            cls.first_resolution_summary = resolve_entities_deterministically(session, reporting_period_id=period.id)
            session.commit()
    ...
    def test_canonical_company_seed_persists_asic_cross_reference_without_abr_review_layer(self) -> None:
        with self.SessionLocal() as session:
            ...
            self.assertEqual(VERMONT_AUS_HOLDCO_CANONICAL_NAME, entity.canonical_name)
            self.assertEqual("company", entity.entity_type)
            self.assertEqual(VERMONT_AUS_HOLDCO_ABN, entity.abn)
            self.assertIsNone(entity.abn_review_source)
            self.assertIsNone(entity.registered_name_on_abr)
            self.assertEqual(VERMONT_AUS_HOLDCO_ACN, entity.acn)
            ...
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_REGISTRATION_DATE, entity.asic_registration_date)
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_NEXT_REVIEW_DATE, entity.asic_next_review_date)
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_CROSS_REFERENCE.record_url, entity.asic_record_url)
            self.assertEqual(ASIC_REVIEW_SOURCE, entity.asic_review_source)
            self.assertEqual(ASIC_REVIEWED_BY, entity.asic_reviewed_by)
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_REVIEWED_AT, entity.asic_reviewed_at)
    ...
    def test_seed_is_idempotent_and_reapplies_only_vermont_asic_fields(self) -> None:
        with self.SessionLocal() as session:
            first = ensure_vermont_aus_holdco_seed(session)
            second = ensure_vermont_aus_holdco_seed(session)
            ensure_vermont_aus_holdco_asic_company_register_cross_reference(session)
    ...
    def test_exact_name_resolution_links_vermont_and_company_detail_stays_honest_with_asic_only_slice(self) -> None:
        with self.SessionLocal() as session:
            rows = session.scalars(
                select(Holding)
                .where(Holding.raw_name == VERMONT_AUS_HOLDCO_CANONICAL_NAME)
            ...
            self.assertEqual(0, self.first_resolution_summary.exact_name_candidates_rejected_due_to_scope_conflict)
            self.assertTrue(all(holding.entity_id == self.entity_id for holding in rows))
            self.assertEqual(self.pre_seed_signatures, TestVermontAusHoldcoSeed._load_vermont_signatures(session))
            ...
            self.assertTrue(detail.history_is_limited)
            self.assertEqual("Linked", detail.entity_confidence_label)
            self.assertEqual(VERMONT_AUS_HOLDCO_ABN, detail.abn)
            self.assertIsNone(detail.abn_review_source)
            self.assertIsNone(detail.registered_name_on_abr)
            self.assertEqual(VERMONT_AUS_HOLDCO_ACN, detail.acn)
            ...
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_REGISTRATION_DATE, detail.asic_registration_date)
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_NEXT_REVIEW_DATE, detail.asic_next_review_date)
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_CROSS_REFERENCE.record_url, detail.asic_record_url)
            self.assertEqual(ASIC_REVIEW_SOURCE, detail.asic_review_source)
            self.assertEqual(ASIC_REVIEWED_BY, detail.asic_reviewed_by)
            self.assertEqual(VERMONT_AUS_HOLDCO_ASIC_REVIEWED_AT, detail.asic_reviewed_at)
